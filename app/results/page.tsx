...
import { calculerResultatsACF, ACFFormData } from '@/lib/acf-calculations'
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    // Essai 1 : données encodées (format actuel du calculator)
    let formData: ACFFormData | null = null
    try {
      formData = JSON.parse(decodeURIComponent(saved))
    } catch {
      try {
    ...
    if (formData) {
      const computed = calculerResultatsACF(formData)
      const level = getLevelFromScore(computed.scoreGlobal)
      const marketPercentile = getMarketPercentile(computed.scoreGlobal)

      const priorities = computed.priorites.map((p, i) => ({
        title: p.titre,
        description: p.description,
        urgency: (i === 0 && computed.scoreGlobal < 40 ? 'critical'
                : i === 0 ? 'high'
                : i === 1 ? 'high'
      ...
      setResults({
        globalScore: computed.scoreGlobal,
        sovereigntyScore: computed.scoreSouverainete,
        ...
        sector: formData.secteur ? (secteurLabels[formData.secteur] || formData.secteur) : undefined,
        formData,
      })
    }
  }, [])
            ...
            <div className="rp-hero-grid">
              ...
              <div>
                ...
                <div className="rp-hero-btns">
                  <button className="rp-btn-g" onClick={() => results.formData && downloadPDF(
                    calculerResultatsACF(results.formData), results.formData
                  )}>
              ...
              <div className="rp-card">
                ...
                <div className="rp-sov">
                  ...
                  <div className="rp-sov-body">
                    <div className="rp-sov-val">{results.sovereigntyScore.toFixed(1)}<span style={{fontSize:'20px',color:'#6b7fa0'}}>/100</span></div>
